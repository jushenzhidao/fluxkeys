...
// 本文件用真实 Postgres 验证 PatchUpstreamKeyState。
//
...
func TestPatchUpstreamKeyState_回显新旧值且只改指定列(t *testing.T) {
	s, ctx := newTestStore(t)
	seedPatchKey(t, s, ctx, "volc_001", "active", "hot", "p_day")

	res, err := s.PatchUpstreamKeyState(ctx, "volc_001", UpstreamKeyPatch{
		Status: strPtr("banned"),
	})
	if err != nil {
		t.Fatalf("PatchUpstreamKeyState: %v", err)
	}
	...
	k, err := s.GetUpstreamKey(ctx, "volc_001")
	if err != nil {
...
func TestPatchUpstreamKeyState_nil字段绝不参与更新(t *testing.T) {
	...
	// 只改 pool，其余全 nil
	if _, err := s.PatchUpstreamKeyState(ctx, "volc_001", UpstreamKeyPatch{
		Pool: strPtr("warm"),
	}); err != nil {
		t.Fatalf("PatchUpstreamKeyState: %v", err)
	}

	k, _ := s.GetUpstreamKey(ctx, "volc_001")
	if k.Status != "banned" {
...
func TestPatchUpstreamKeyState_显式空串会真的清空(t *testing.T) {
...
func TestPatchUpstreamKeyState_expectedStatus不匹配时不写入(t *testing.T) {
	s, ctx := newTestStore(t)
...
func TestPatchUpstreamKeyState_expectedStatus匹配时正常写入(t *testing.T) {
	s, ctx := newTestStore(t)
	seedPatchKey(t, s, ctx, "volc_001", "active", "hot", "")
...
func TestPatchUpstreamKeyState_RejectStatusFrom拦截终态(t *testing.T) {
	s, ctx := newTestStore(t)
	...
	for _, keyID := range []string{"volc_banned", "volc_invalid"} {
		_, err := s.PatchUpstreamKeyState(ctx, keyID, UpstreamKeyPatch{
		...
		k, _ := s.GetUpstreamKey(ctx, keyID)
		if k.Status == "active" {
...
func TestPatchUpstreamKeyState_守卫为空时不拦截(t *testing.T) {
...
func TestPatchUpstreamKeyState_Key不存在返回ErrNotFound(t *testing.T) {
	// 与 ErrPreconditionFailed 必须可区分: 两者在 UPDATE 层面都是影响 0 行，
	// 对外一个 404 一个 409。
	s, ctx := newTestStore(t)
	res, err := s.PatchUpstreamKeyState(ctx, "volc_nope", UpstreamKeyPatch{Status: strPtr("banned")})
...
func TestPatchUpstreamKeyState_同值重放幂等(t *testing.T) {
	s, ctx := newTestStore(t)
	seedPatchKey(t, s, ctx, "volc_001", "active", "hot", "p_day")

	for i := 0; i < 3; i++ {
		res, err := s.PatchUpstreamKeyState(ctx, "volc_001", UpstreamKeyPatch{Pool: strPtr("cold")})
		if err != nil {
...
func TestPatchUpstreamKeyState_无待更新字段直接报错(t *testing.T) {
	s, ctx := newTestStore(t)
	seedPatchKey(t, s, ctx, "volc_001", "active", "hot", "")
	if _, err := s.PatchUpstreamKeyState(ctx, "volc_001", UpstreamKeyPatch{}); err == nil {
		t.Error("三个字段全 nil 时应报错，否则调用方无法区分「改了」与「什么都没改」")
	}
	if _, err := s.PatchUpstreamKeyState(ctx, "", UpstreamKeyPatch{Status: strPtr("banned")}); err == nil {
		t.Error("空 key_id 应报错")
	}
}

func TestPatchUpstreamKeyState_并发下乐观并发控制严格成立(t *testing.T) {
	...
	for i := 0; i < n; i++ {
		...
		go func(i int) {
			...
			_, err := s.PatchUpstreamKeyState(ctx, "volc_001", UpstreamKeyPatch{
...
func TestPatchUpstreamKeyState_不触碰密文与出口绑定(t *testing.T) {
	...
	before, _ := s.GetUpstreamKey(ctx, "volc_001")
	...
	after, _ := s.GetUpstreamKey(ctx, "volc_001")
